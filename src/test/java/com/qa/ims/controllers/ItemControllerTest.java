...
public class ItemControllerTest {
	...
	public void testCreate() {
		final String ItemName = "Cat";
		final Double Price = 23.99;
	...
	public void testReadAll() {
		...
		items.add(new Item(1L, "Dog", 12.99));
	...
	public void testUpdate() {
		Item updated = new Item(1L, "Cat", 23.99);

		Mockito.when(this.utils.getLong()).thenReturn(1L);
		Mockito.when(this.utils.getString()).thenReturn(updated.getItemName());
		Mockito.when(this.utils.getDouble()).thenReturn(updated.getPrice());
		Mockito.when(this.dao.update(updated)).thenReturn(updated).thenReturn(updated);
